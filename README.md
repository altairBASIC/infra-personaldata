...
graph TB
    ...
    subgraph CONTAINERS["Red de Contenedores OCI"]
        subgraph INTERNAL["Red: internal (sin egress)"]
            ...
            subgraph PIPELINE["Contenedor: pipeline"]
                INGESTA["[1] Ingesta<br/>mbox → MinIO Bronze"]
                NORMALIZA["[2] Normalización<br/>Bronze → Polars DataFrame<br/>encoding, HTML→text, UUID5"]
                CALIDAD["[3] Calidad<br/>6 reglas declarativas<br/>@regla registry"]
                EMBEDDINGS["[4] Embeddings<br/>sentence-transformers<br/>chunking → Chroma"]
                GOLD["[5] Gold<br/>DuckDB → 5 tablas<br/>analíticas Parquet"]
                LINAJE["📋 linaje.json<br/>(thread-safe)"]
            end
        end
        ...
        subgraph EXTERNAL["Contenedor api — red internal, puerto 8000 publicado (sin egress)"]
            subgraph API["Contenedor: api"]
                ...
                BUSQUEDA["POST /search<br/>(Chroma similarity search)"]
                HEALTH["GET /health"]
            end
        end
        ...
        subgraph VOLUMES["Volúmenes Nombrados"]
            ...
            CACHE_VOL["model_cache<br/>HF models"]
        end
    end
    ...
    NORMALIZA --> CALIDAD
    CALIDAD -->|"valid rows"| EMBEDDINGS
    CALIDAD -->|"valid rows"| GOLD
    EMBEDDINGS -->|"upsert chunks"| CHROMA_VOL
    GOLD -->|"write"| GOLD_VOL
    NORMALIZA -->|"write Parquet"| SILVER_VOL
    PIPELINE --> LINAJE

    SILVER_VOL -->|":ro"| CONSULTA
    GOLD_VOL -->|":ro"| CONSULTA
    CHROMA_VOL -->|":ro"| BUSQUEDA
    CACHE_VOL --> EMBEDDINGS
